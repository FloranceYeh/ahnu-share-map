...
const allowedOrigins = new Set(
  (Deno.env.get('ALLOWED_ORIGINS') || 'https://map.florance.top,http://localhost:5173,http://127.0.0.1:5173')
    .split(',')
    .map((origin) => origin.trim())
    .filter(Boolean),
)
const allowedTurnstileHostnames = new Set(
  (Deno.env.get('TURNSTILE_ALLOWED_HOSTNAMES') || 'map.florance.top,localhost,127.0.0.1')
    .split(',')
...
const fail = (message: string): never => { throw new Error(`INPUT:${message}`) }

function stringValue(value: unknown, field: string, maxLength: number, required: true): string
function stringValue(value: unknown, field: string, maxLength: number, required?: false): string | null
function stringValue(value: unknown, field: string, maxLength: number, required = false) {
  ...
  if (trimmed.length > maxLength) fail(`${field}长度不能超过${maxLength}个字符`)
  return trimmed || null
}

function normalizePayload(payload: unknown, submissionId: string | null) {
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) fail('投稿数据格式错误')
  const source = payload as Record<string, unknown>
  const latitude = Number(source.latitude)
  const longitude = Number(source.longitude)
  if (!Number.isFinite(latitude) || latitude < -90 || latitude > 90) fail('纬度无效')
  if (!Number.isFinite(longitude) || longitude < -180 || longitude > 180) fail('经度无效')

  const customSource = source.custom_details
  if (customSource !== undefined && (!customSource || typeof customSource !== 'object' || Array.isArray(customSource))) fail('自定义项格式错误')
  const customDetails: Record<string, unknown> = {}
  const customEntries = Object.entries((customSource || {}) as Record<string, unknown>)
  if (customEntries.length > 20) fail('自定义项不能超过20个')
  for (const [key, value] of customEntries) {
    if (key === 'duplicate_candidates') continue
    if (key.length > 50) fail('自定义项名称过长')
    if (key === 'images') {
      if (!Array.isArray(value) || value.length > 3 || value.some((path) => typeof path !== 'string' || !IMAGE_PATH_PATTERN.test(path) || !submissionId || !path.startsWith(`${submissionId}/`))) fail('图片路径无效')
      customDetails.images = value
      continue
    }
    if (typeof value !== 'string' || value.length > 500) fail('自定义项内容无效')
...
Deno.serve(async (request) => {
  ...
  if (request.method !== 'POST') return json({ error: '只支持 POST 请求' }, 405, origin)

  try {
    const contentLength = Number(request.headers.get('content-length') || 0)
    if (contentLength > MAX_BODY_BYTES) return json({ error: '投稿数据过大' }, 413, origin)
    const rawText = await request.text()
    if (new TextEncoder().encode(rawText).byteLength > MAX_BODY_BYTES) return json({ error: '投稿数据过大' }, 413, origin)
    let rawBody: unknown
    try { rawBody = JSON.parse(rawText) } catch { fail('请求格式错误') }
    if (!rawBody || typeof rawBody !== 'object' || Array.isArray(rawBody)) fail('请求格式错误')
    const body = rawBody as Record<string, unknown>
    const turnstileToken = stringValue(body.turnstileToken, '人机验证', 4096, true)
    const fingerprint = stringValue(body.fingerprint, '设备标识', 128, true)
    const rawSubmissionId = body.submissionId
    const submissionId = rawSubmissionId === undefined || rawSubmissionId === null || rawSubmissionId === '' ? null : stringValue(rawSubmissionId, '投稿编号', 36)
    ...
    if (normalizedPayload.custom_details.images && !submissionId) fail('图片投稿编号缺失')
    ...
    if (!verification.success || !hostname || !allowedTurnstileHostnames.has(hostname)) return json({ error: '人机验证失败' }, 403, origin)
    ...
    if (normalizedPayload.category_id) {
      const { data: category, error: categoryError } = await admin.from('categories').select('id').eq('id', normalizedPayload.category_id).eq('is_active', true).maybeSingle()
    ...
    const { data: nearby, error: nearbyError } = await admin.from('places').select('id,name').gte('latitude', normalizedPayload.latitude - 0.0015).lte('latitude', normalizedPayload.latitude + 0.0015).gte('longitude', normalizedPayload.longitude - 0.0015).lte('longitude', normalizedPayload.longitude + 0.0015).limit(5)
    if (nearbyError) throw nearbyError
    const duplicateCandidates = (nearby || []).filter((place) => place.name.includes(normalizedPayload.name) || normalizedPayload.name.includes(place.name)).map((place) => ({ id: place.id, name: place.name }))
    const insertPayload = {
      ...normalizedPayload,
      ...(submissionId ? { id: submissionId } : {}),
      custom_details: { ...normalizedPayload.custom_details, duplicate_candidates: duplicateCandidates },
      status: 'pending',
    }
    ...
    if (message.startsWith('INPUT:')) return json({ error: message.slice(6) }, 400, origin)
