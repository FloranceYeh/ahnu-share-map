...
import Turnstile, { clearPreloadedToken, getPreloadedToken } from './Turnstile'
...
export default function SubmissionForm({ draft, setDraft, categories, detailFields, onClose, onSubmitted, adminMode = false }) {
  ...
  const submit = async (turnstileToken = '') => {
    ...
    } finally { setSubmitting(false); setVerifying(false); pendingSubmitRef.current = false; if (!adminMode) { clearPreloadedToken(); turnstileRef.current?.reset() } }
  }
  const handleTurnstileToken = (token) => {
    setVerifying(false)
    if (pendingSubmitRef.current && token) {
  ...
  const requestSubmit = () => {
    ...
    setError('')
    const preloadedToken = getPreloadedToken()
    if (preloadedToken) { submit(preloadedToken); return }
    pendingSubmitRef.current = true
    setVerifying(true); turnstileRef.current?.execute()
  }
