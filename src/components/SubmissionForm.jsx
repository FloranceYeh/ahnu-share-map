...
import {
  createApprovedPlaceWithImages,
  submitPlaceWithImages,
} from "../lib/supabase";
...
export default function SubmissionForm({
  ...
  const submit = async () => {
    ...
    try {
      ...
      const code = adminMode
        ? await createApprovedPlaceWithImages(payload, files)
        : await submitPlaceWithImages(payload, files, turnstileToken);
      setQueryCode(code);
      onSubmitted?.(code);
    } catch (submitError) {
    ...
    return (
      <aside className="debug-panel submission-panel">
        <div className="drawer-heading">
          <div>
            <p className="section-kicker">
              {adminMode ? "PUBLISHED" : "SUBMITTED"}
            </p>
            <h2>{adminMode ? "地点已发布" : "已提交审核"}</h2>
          </div>
          <button
        ...
        <div className="submission-success">
          ...
          <strong>{queryCode}</strong>
          <button onClick={() => navigator.clipboard.writeText(queryCode)}>
            复制查询码
          </button>
        </div>
      </aside>
  ...
  return (
    <aside className="debug-panel submission-panel">
      <div className="drawer-heading">
        <div>
          <p className="section-kicker">
            {adminMode ? "ADMIN PLACE" : "SHARE A PLACE"}
          </p>
          <h2>{adminMode ? "管理员加点" : "新增推荐点"}</h2>
        </div>
        <button
          className="drawer-close"
          onClick={onClose}
      ...
      <div className="debug-form">
        ...
        {!adminMode && <Turnstile onToken={setTurnstileToken} />}
        ...
        <button
          ...
          disabled={
            ...
            (!adminMode && !turnstileToken)
          }
        >
          {submitting ? "提交中…" : adminMode ? "直接发布" : "提交审核"}
        </button>
        <textarea
