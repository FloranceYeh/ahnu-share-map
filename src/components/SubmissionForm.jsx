...
import { clearPreloadedToken, getPreloadedToken, refreshPreloadedTokenNow, subscribePreloadedToken } from './Turnstile'
...
export default function SubmissionForm({ draft, setDraft, categories, detailFields, onClose, onSubmitted, adminMode = false }) {
  ...
  const [turnstileToken, setTurnstileToken] = useState(() => getPreloadedToken())
  useEffect(() => subscribePreloadedToken(setTurnstileToken), [])
  ...
  const submit = async () => {
    ...
    } finally { setSubmitting(false); if (!adminMode) { clearPreloadedToken(); refreshPreloadedTokenNow() } }
  }
